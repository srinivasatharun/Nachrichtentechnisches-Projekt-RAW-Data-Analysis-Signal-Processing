...
DataPath = 'D:/Hochschule Wismar/Summer 2023/Telecommuincation Project/Final_Project/Task_1-4/'
...
RG, FG = np.meshgrid(ranges, freq, indexing='ij')  

# Spectrum calculation for all receivers
Spectrum = data*0.0
for rx in range (noRx):
    for rg in range(noRG):
        Spectrum[rg, :, rx] = np.fft.fftshift( np.fft.fft( data[rg,:,rx] ) )/ len(freq) 

# --------------------------------------------------------- TASK 1 ---------------------------------------------------------------

# Plot for Magnitude and Phase of Spectra Receiver - 0

plt.figure(1)
plt.subplot(1, 2, 1)
plt.pcolor(FG/1000, RG, 10*np.log10(np.abs(Spectrum[:,:,0])), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Magnitude Spectra for Receiver - 1')
plt.colorbar()
plt.subplot(1, 2, 2)
plt.pcolor(FG/1000, RG, np.angle(Spectrum[:,:,0]), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Phase Spectra for Receiver - 1')
plt.colorbar()

plt.tight_layout()  
plt.show()

# Plot for Magnitude and Phase of Spectra Receiver - 4

plt.figure(2)
plt.subplot(1, 2, 1)
plt.pcolor(FG/1000, RG, 10*np.log10(np.abs(Spectrum[:,:,3])), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Magnitude Spectra for Receiver - 4')
plt.colorbar()
plt.subplot(1, 2, 2)
plt.pcolor(FG/1000, RG, np.angle(Spectrum[:,:,3]), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Phase Spectra for Receiver - 4')
plt.colorbar()
...
superposition = np.sum(data[:, :, 1:], axis=-1)

spectrum_superposition = superposition * 0.0
for gate in range(noRG):
    spectrum_superposition[gate, :] = np.fft.fftshift(np.fft.fft(superposition[gate, :])) / len(freq)

# Plot for Magnitude and Phase of Superposition of Spectra 2-5

plt.figure(3)
plt.subplot(1, 2, 1)
plt.pcolor(FG/1000, RG, 10*np.log10(np.abs(spectrum_superposition)), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Magnitude - Superposition')
plt.colorbar()
plt.subplot(1, 2, 2)
plt.pcolor(FG/1000, RG, np.angle(spectrum_superposition), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Phase - Superposition')
plt.colorbar()
...
combination_Spectra = np.sum(Spectrum[:, :, 1:], axis=-1)

# Plot for Magnitude and Phase of Combination of Individual Spectra 2-5

plt.figure(4)
plt.subplot(1, 2, 1)
plt.pcolor(FG/1000, RG, 10*np.log10(np.abs(combination_Spectra)), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Magnitude - Combination_Spectra')
plt.colorbar()
plt.subplot(1, 2, 2)
plt.pcolor(FG/1000, RG, np.angle(combination_Spectra), cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Phase - Combination_Spectra')
plt.colorbar()
...
plt.colorbar()
plt.subplot(1, 2, 2)
phases1=np.angle(spectrumn_nci[:,:,3])/np.pi*180
SNRsel1=10*np.log10(abs(spectrumn_nci[:,:,3])/2)<-5   
phases1[SNRsel1]="nan"
...
# Plot for Magnitude and Phase of Incoherent Integrations Receiver - 4

plt.figure(6)
plt.subplot(1, 2, 1)
ampln=10*np.log10(np.abs(spectrumn_nci[:,:,3]))
SNRsel=ampln<-5   
ampln[SNRsel]="nan"
plt.pcolor(FG_nci/1000, RG_nci, ampln, cmap='jet', shading='auto')
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Magnitude - Incoherent Integrations (Receiver - 4)')
plt.colorbar()
plt.subplot(1, 2, 2)
phases1=np.angle(spectrumn_nci[:,:,3])/np.pi*180
SNRsel1=10*np.log10(abs(spectrumn_nci[:,:,3])/2)<-5   
...
plt.xlabel('Frequency')
plt.ylabel('Range')
plt.title('Phase - Incoherent Integrations (Receiver - 4)')
plt.colorbar()
...
XSpectrn[:,:,0] = Spectrn[:,:,1] * np.conj(Spectrn[:,:,4]) #2-5

# Plot for Magnitude and Phase of Cross_Spectrum (Incoherent Integrations)  Receiver - 2-5
